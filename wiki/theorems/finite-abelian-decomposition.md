...
> **依赖关系小结**：本证明依赖链为「基本群论（群同态、基数论证、正合列分裂）⇒ Claim 1（CRT）与 Claim 2（特征标群直积）⇒ 单位根分类（[[ch7-ex5-circle-character|Ch.7 Ex.4]]，$e(1)$ 为 $N$ 次单位根）⇒ Claim 3（$\widehat{\mathbb{Z}(N)}\cong\mathbb{Z}(N)$）⇒ (iii) $\widehat{G}\cong G$（由 Claim 2、3 与结构定理 (ii) 拼装）」。结构定理 (ii) 的存在性依赖「有限阿贝尔群的正合列分裂」这一标准代数事实（书中未证），不变因子由素幂分解 + CRT 反向合并导出；依赖方向为 (ii) ⇒ (iii)，(ii) 的证明不依赖 (iii)，依赖图无环。本证明除 Ch. 7 Ex. 4 与标准群论事实外无定理级依赖（不依赖 Fourier 分析工具）。
