...
> **依赖关系小结**: 本证明依赖链为「[[cauchy-theorem|Cauchy 定理]] ⇒ [[residue-theorem|留数定理]] ⇒ 辐角原理」：第一阶段做 $f'/f$ 的局部结构分析（零点处局部 $f(z) = (z-z_0)^n g(z)$，得 $f'/f = n/(z-z_0) + g'/g$，故 $f'/f$ 有 1 阶极点且留数为 $n$；极点处同理留数为 $-m$，均为初等计算），第二阶段把留数定理（Stein Corollary 2.2）应用于 $f'/f$ 求和得 $Z - P$。本证明不依赖 Rouché 定理等后续定理，依赖图无环。
